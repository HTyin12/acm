class fastIO {
private:
    char ibuf[(1<<23)], *p1 = ibuf, *p2 = ibuf, obuf[(1<<23)], *p3 = obuf, sta[50];
    bool file_end = false;
   
    char get() {
        return p1 == p2 && (p2 = (p1 = ibuf) + fread(ibuf, 1, (1<<23), stdin), p1 == p2) ? (file_end = true), char(EOF) : *p1++;
    }
   
    void put(const char x) {
        p3 - obuf < (1<<23) ? *p3++ = x : (fwrite(obuf, p3 - obuf, 1, stdout), p3 = obuf, *p3++ = x);
    }
...
const int Y = 1 << 23;
#define gc() (p1 == p2 && (p2 = (p1 = a1) + fread(a1, 1, Y, stdin), p1 == p2) ? EOF : *p1++)
#define pc(x) (pp - a2 == Y ? (fwrite(a2, 1, Y, stdout), pp = a2, *pp++ = x) : *pp++ = x)
class fastIO
{
private:
    char a1[Y], *p1 = a1, *p2 = a1, a2[Y], *pp = a2;
...
public:
    ...
    ~fastIO() { fwrite(a2, 1, pp - a2, stdout); }
} FastIO;
#define cin FastIO
#define cout FastIO