...
vector<pair<long long,int>>pollard_rho(long long n){//所有质因子及个数
	if(n==1) return {};
...
vector<long long>find_divisors(long long n){//所有约数
	vector<pair<long long,int>>res=pollard_rho(n);
	vector<long long>p;p.push_back(1);
	for(int i=0;i<res.size();i++){
		vector<long long>pp;long long tmp=1;
		for(int j=0;j<=res[i].second;j++){
			for(auto it:p)pp.push_back(it*tmp);
			tmp*=res[i].first;
		}p=pp;
	}
	return p;
